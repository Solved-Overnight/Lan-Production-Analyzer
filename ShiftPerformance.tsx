...
const SupervisorFridayTable = ({ name, color, rows, shiftKey }: { name: string, color: 'indigo' | 'amber', rows: any[], shiftKey: 'yousuf' | 'humayun' }) => {
  ...
  return (
    <div className="bg-app-card rounded-lg border border-app-border shadow-sm overflow-hidden flex flex-col h-full">
      <div className={`px-4 py-2 border-b flex items-center justify-between shrink-0 ${isIndigo ? 'bg-indigo-500/10 border-indigo-100' : 'bg-amber-500/10 border-amber-100'}`}>
         <div className="flex items-center gap-2">
            <User size={14} className={isIndigo ? 'text-indigo-600' : 'text-amber-600'} />
            <h4 className={`text-[9px] font-black uppercase tracking-widest ${isIndigo ? 'text-indigo-900' : 'text-amber-900'}`}>{name} Friday Log</h4>
         </div>
      </div>
      <div className="overflow-y-auto custom-scrollbar flex-1 relative">
        <table className="w-full text-center text-[9px] border-collapse">
           <thead className="bg-app-bg text-app-text-muted font-black uppercase tracking-tighter border-b border-app-border sticky top-0 z-10 shadow-sm">
              <tr>
                 <th className="p-1 border-r border-app-border">Date</th>
                 <th className="p-1 border-r border-app-border">From</th>
                 <th className="p-1 border-r border-app-border">To</th>
                 <th className="p-1 border-r border-app-border">Hours</th>
              ...
              {fridayData.map((r, i) => {
                 ...
                 return (
                    <tr key={i} className="hover:bg-app-bg/50 font-bold transition-colors">
                       <td className="p-1 border-r border-app-border text-slate-900 whitespace-nowrap">{r.day} (F)</td>
                       <td className="p-1 border-r border-app-border text-slate-500">{from}</td>
                       <td className="p-1 border-r border-app-border text-slate-500">{to}</td>
                       <td className={`p-1 border-r border-app-border font-black ${isIndigo ? 'text-indigo-600' : 'text-amber-600'}`}>{hrs.toFixed(1)}h</td>
                       <td className="p-1 text-emerald-600">{count.toFixed(2)}</td>
                    </tr>
                 );
              })}
              {fridayData.length === 0 && (
      ...
      {fridayData.length > 0 && (
        <div className={`shrink-0 font-black uppercase text-[3.2px] border-t sticky bottom-0 z-20 ${isIndigo ? 'bg-indigo-100 text-indigo-900 border-indigo-200' : 'bg-amber-100 text-amber-900 border-amber-200'}`}>
          <table className="w-full text-center border-collapse">
            <tbody>
              <tr>
                <td className="py-0.5 border-r border-app-border/20 text-left pl-3 uppercase tracking-tighter" colSpan={3}>MONTHLY AGGREGATE</td>
                ...
                <td className="py-0.5 w-[15%] text-center font-black tracking-tighter">{(fridayData.reduce((s, r) => s + r[shiftKey].dayCount, 0) / fridayData.length).toFixed(2)} AVG</td>
              </tr>
            </tbody>
          </table>
        </div>
