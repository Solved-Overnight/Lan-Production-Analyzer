...
import { 
  ...
  Zap, Timer, ClipboardCheck, ChevronRight, Menu
} from 'lucide-react';
...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="h-screen flex flex-col md:flex-row bg-app-bg text-app-text transition-colors duration-200 overflow-hidden">
      ...
      <nav className={`w-full ${isCollapsed ? 'md:w-20' : 'md:w-64'} bg-app-card border-b md:border-r border-app-border p-4 shrink-0 transition-all duration-300 ease-in-out flex flex-col shadow-sm z-20 relative`}>
        ...
        {/* BRANDING SECTION */}
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3.5'} mb-6 px-1 py-1 select-none overflow-hidden`}>
          ...
          <div className={`flex flex-col transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 pointer-events-none' : 'w-auto opacity-100'}`}>
            ...
            return (
              <li key={item.path}>
                <Link
                  ...
                  className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3.5'} px-3.5 py-3 rounded-md text-[13px] font-bold transition-all group ${
                    isActive 
        ...
        <div className={`mt-auto pt-5 px-1 border-t border-app-border transition-all duration-300 ${isCollapsed ? 'flex justify-center' : ''}`}>
          {isCollapsed ? (
             <div className="w-8 h-8 rounded-md bg-app-accent/20 flex items-center justify-center text-[8px] font-bold">HQ</div>
          ) : (
